...
DockerImage=192.168.6.184:5000/ithappens/$DockerAppName:$DockerImageVersionLatest
...
DockerImageName=qt-$DockerAppEnv-$DockerAppName
...
docker build . -t qt-$DockerAppName #--build-arg DIR_APP_RELEASE="$DirAppReleaseVersion"

IdImage=$(docker images -q qt-$DockerAppName)
